      ...
      <View style={styles.container}>
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior="padding" keyboardVerticalOffset={0}>
          <FlatList
            ...
            ListEmptyComponent={<View style={{ alignItems: 'center', padding: 32 }}><Text style={{ fontFamily: 'Inter_400Regular', fontSize: 14, color: '#9BA3AF' }}>Start a consultation</Text></View>}
          />
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.headerSub}>Chat with patients and donors</Text>
      </LinearGradient>
      <FlatList
        data={myConvs}
        keyExtractor={item => item.id}
        contentContainerStyle={{ padding: 16, paddingBottom: 100 }}
        ListEmptyComponent={<EmptyState icon="chat-outline" title="No consultations" subtitle="Conversations will appear here" />}
        renderItem={({ item }) => {
          ...
          return (
            <Pressable style={styles.convCard} onPress={() => setSelectedConvId(item.id)}>
              <View style={styles.convAvatar}><Text style={styles.convAvatarText}>{otherName[0]}</Text></View>
              <View style={{ flex: 1 }}>
                <Text style={styles.convName}>{otherName}</Text>
                <Text style={styles.convLastMsg} numberOfLines={1}>{item.lastMessage || 'Tap to start'}</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  chatHeader: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 16, paddingBottom: 16 },
  backBtn: { padding: 4 },
  chatAvatar: { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(255,255,255,0.25)', alignItems: 'center', justifyContent: 'center' },
  ...
  convCard: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#fff', borderRadius: 16, padding: 14, marginBottom: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 },
  convAvatar: { width: 48, height: 48, borderRadius: 24, backgroundColor: '#2EC4B6', alignItems: 'center', justifyContent: 'center' },
  ...
  bubble: { maxWidth: '78%', padding: 12, borderRadius: 16, borderTopLeftRadius: 4, backgroundColor: '#fff', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 3, elevation: 1 },
  ...
  inputRow: { flexDirection: 'row', alignItems: 'flex-end', gap: 10, padding: 12, paddingTop: 8, backgroundColor: '#fff', borderTopWidth: 1, borderTopColor: '#F0F4F8' },
  msgInput: { flex: 1, fontFamily: 'Inter_400Regular', fontSize: 15, color: '#1E1E1E', backgroundColor: '#F7F9FB', borderRadius: 20, paddingHorizontal: 16, paddingVertical: 10, maxHeight: 100, minHeight: 44 },
  sendBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#2EC4B6', alignItems: 'center', justifyContent: 'center' },
});
