...
export default function DoctorProfile() {
  ...
  const notifTypeColor: Record<string, string> = {
    request: '#C62828', accepted: '#2563EB', approved: '#2EC4B6', completed: '#16A34A', general: '#9BA3AF',
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        >
          ...
          <View style={styles.roleBadge}>
            ...
            <Text style={styles.roleBadgeText}>Medical Doctor</Text>
          </View>
        </LinearGradient>
        ...
        <View style={styles.content}>
          <View style={styles.infoCard}>
            {[
              ...
              { icon: 'business-outline', label: 'Hospital', value: user.hospital || 'N/A' },
              { icon: 'card-outline', label: 'License Number', value: user.licenseNumber || 'N/A' },
          ...
          {notifications.length === 0 ? (
            <EmptyState icon="bell-off" title="No notifications" subtitle="Request alerts will appear here" />
          ) : (
...
const styles = StyleSheet.create({
  ...
  markAllRead: { fontFamily: 'Inter_500Medium', fontSize: 13, color: '#2EC4B6' },
  ...
  notifCardUnread: { backgroundColor: '#F0FDFC', borderLeftWidth: 3, borderLeftColor: '#2EC4B6' },
  ...
  unreadDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#2EC4B6', marginTop: 4 },
