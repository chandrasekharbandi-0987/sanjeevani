...
export default function DonorChat() {
  ...
  const { user } = useAuth();
  const { conversations, messages, allUsers, getOrCreateConversation, sendMessage } = useData();
      ...
      <View style={styles.container}>
        ...
        >
          <FlatList
            ...
            inverted
            contentContainerStyle={{ padding: 16, gap: 10, flexDirection: 'column-reverse' }}
            renderItem={({ item }) => {
            ...
            ListEmptyComponent={
              <View style={{ alignItems: 'center', padding: 32 }}>
                <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 14, color: '#9BA3AF' }}>
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.headerSub}>Chat with recipients</Text>
      </LinearGradient>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState icon="chat-outline" title="No conversations" subtitle="Accept a blood request to start chatting with a recipient" />
        }
        renderItem={({ item }) => {
          ...
          return (
            ...
            >
              ...
              {item.lastMessageTime && (
                <Text style={styles.convTime}>{timeAgo(item.lastMessageTime)}</Text>
              )}
...
const styles = StyleSheet.create({
  ...
  bubbleMine: { backgroundColor: '#C62828', borderTopLeftRadius: 16, borderTopRightRadius: 4 },
